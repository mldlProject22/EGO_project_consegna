...
def main():
  ...
  params = { 
    ...
    'beta_t': args.beta[1],
    'beta_s': args.beta[2],
    'loss_r': args.loss_weights[0],
    'loss_t': args.loss_weights[1],
    'loss_s': args.loss_weights[2]
    }

    

  print("Mean Accuracy: ",test(params))
...
def test(params):
  ...
  for domain_shift in d_lst:
    ...
    model = ta3n_model(frame_aggregation=aggregation,modality=modality).cuda()
    ...
    loss_weights=[params['loss_r'] if place_adv[0]=="Y" else 0 ,params['loss_t'] if place_adv[1]=="Y" else 0 ,
    params['loss_s'] if place_adv[2]=="Y" else 0  ]
...
def load_data(is_train,domain_shift):
  ...
  if modality!="All":
    ...
    data_path_target_train_flow = '/content/drive/MyDrive/Progetto_2022/Features/Flow/'+domain_shift+'_train.pkl' #da mettere flow
    ...
    target_ds_flow = loaderFeat(data_path_target_train_flow,dataset_path_target_train,"Flow") #da mettere flow
