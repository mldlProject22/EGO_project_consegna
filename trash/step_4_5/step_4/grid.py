...
def main():
  ...
  print('***BEST TRIAL***')
  print('mean accuracy: ', study.best_value)
  for key, value in best_trial.params.items():
    print("{}: {}".format(key, value))

  f = open("/content/EGO_Project/step_4/res.txt", "a")
  f.write("\n")
  ...
  f.write("\n")
  f.write("accuracy "+str(study.best_value))
  f.write("\n")
  f.write("params "+str(best_trial.params.items()))
  f.write("\n")
  f.close()
...
def to_max(params):
  ...
  for domain_shift in d_lst:
    ...
    val=test(params,domain_shift)

    top1.append(val)

    
    print(domain_shift, ' Accuracy: ', val)
...
def objective(trial):
  if use_attn!="none":
    params = { 
    'gamma' : trial.suggest_loguniform('gamma', 0.001,0.05),
    }

  else:
    params = { 
    'beta' : trial.suggest_loguniform('beta', 0.1,0.8),
    'loss' : trial.suggest_loguniform('loss', 0.1,0.8)
    }

  top1_val = to_max(params)
...
def load_data(is_train,domain_shift):
  ...
  if modality=="RGB":
    ...
    data_path_target_train_flow = '/content/drive/MyDrive/Progetto_2022/Features/RGB/'+domain_shift+'_train.pkl' #da mettere flow
    ...
    target_ds_flow = loaderFeat(data_path_target_train_flow,dataset_path_target_train,"RGB") #da mettere flow
  ...
  '''
    ...
    target_ds_flow = loaderFeat(data_path_target_train_flow,dataset_path_target_train,"RGB") #riga da eliminare 
   
    test_ds_flow = loaderFeat(data_path_test_flow,dataset_path_test,"Flow" )


    source_ds = source_ds_flow.merge(source_ds_rgb)
    print(np.shape(source_ds.data_feat))
    target_ds = target_ds_flow.merge(target_ds_rgb)
    print(np.shape(target_ds.data_feat))
    test_ds = test_ds_flow.merge(test_ds_rgb)
    print(np.shape(source_ds.data_feat))
  ...
  #drop last incrementa le performance di 0.5 per avgpool 
