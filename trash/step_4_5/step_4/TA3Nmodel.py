...
class ta3n_model(nn.Module):
  ...
  def _prepare_DA(self, num_class): # convert the model to DA framework
    ...
    normal_(self.fc_feature_shared_source.weight, 0, std) #CHIEDERE A CHIARA
    ...
    self.fc_classifier_source = nn.Linear(feat_frame_dim, num_class)  #FC_CLASSIFIER_SOURCE = FC_CLASSIFIER_SOURCE_VERB
  ...
  # ------ video-level layers (source layers + domain layers) ------#
    ...
    self.fc_classifier_video_source = nn.Linear(feat_video_dim, num_class)  #fc_classifier_video_source = fc_classifier_video_verb_source
  ...
  def forward(self, input_source, input_target, beta, mu, reverse):
    ...
    feat_fc_source, feat_fc_target = self.domainAlign(feat_fc_source, feat_fc_target, True, 'shared', self.alpha.item(), num_segments, 1)
