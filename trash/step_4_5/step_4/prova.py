...
logger=SummaryWriter("/content/EGO_Project/step_4/log")

def log_gradients_in_model(model,logger,step):
  for tag,value in model.named_parameters():
    if value.grad is not None:
      logger.add_histogram(tag+"\grad",value.grad,step)
...
def main():
  ...
  for ep in range(epochs):
    ...
    train_loss = train(model, criterion, criterion_domain, optimizer, ep, beta, gamma, mu)
  ...
  f.write("\n")
  f.write("loss_weights "+str(loss_weights))
  f.write("\n")
  f.write("top1 accuracy "+str(top1_val))
  f.write("\n")
  f.write("top5 accuracy "+str(top5_val))
  f.write("\n")
...
def train(model, criterion, criterion_domain, optimizer, ep, beta, gamma, mu):
