...
def main():
 ...
 if(model_type == 'i3d'): model = I3Dclassifier.i3d_classifier(input_size = 5*1017, n_classes = n_classes, p = param1['p']).cuda()
 else: model = TRNclassifier.RelationModuleMultiScaleWithClassifier(img_feature_dim = 2048, num_frames = 5, num_class = n_classes, p = param1['p']).cuda()
 

 print("ACCURACY: ",train_test(param1,param2, model),"PARAMETERS: ",param1)
...
def train_test(param1,param2,model):
  ...
  for ep in range(EPOCHS):
    ...
    if ep<=ep/2:
      loss_train, top1_acc_train = train(param1,model)
    else:
      loss_train, top1_acc_train = train(param2,model)  
