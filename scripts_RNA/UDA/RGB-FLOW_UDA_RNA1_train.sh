...
N=UDA_RGB-Flow_RNA-radius$radius # name exp
...
--radius $radius \
--UDA \
--channels_events 3 >> "$path_log"_"$N"_"$source"_"$target".txt
...
--num_clips_test 5 \
--last \
--resume_from  $path_resume/saved_models/$N-$source-$target/ >> "$path_log"_"$N"_"$source"-"$target".txt
